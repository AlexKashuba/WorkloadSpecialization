...
RC index_btree::init(uint64_t part_cnt) {
  ...
  // the index tree of each partition musted be mapped to corresponding l2
  // slices
...
RC index_btree::index_read(idx_key_t key, itemid_t *&item, uint64_t thd_id,
                           int64_t part_id) {
  RC rc = Abort;
  glob_param params;
  assert(part_id != -1);
...
RC index_btree::make_node(uint64_t part_id, bt_node *&node) {
  ...
  new_node->keys = (idx_key_t *)mem_allocator.alloc(
      (order - 1) * sizeof(idx_key_t), part_id);
  new_node->pointers =
      (void **)mem_allocator.alloc(order * sizeof(void *), part_id);
...
RC index_btree::start_new_tree(glob_param params, idx_key_t key,
                               itemid_t *item) {
  RC rc;
...
RC index_btree::find_leaf(glob_param params, idx_key_t key,
                          idx_acc_t access_type, bt_node *&leaf) {
  bt_node *last_ex = NULL;
...
RC index_btree::find_leaf(glob_param params, idx_key_t key,
  ...
  // c is leaf
  // at this point, if the access is a read, then only the leaf is latched by
  // LATCH_SH if the access is an insertion, then the leaf is sh latched and
  // related nodes in the tree are ex latched.
  if (access_type == INDEX_INSERT) {
    if (upgrade_latch(c) != RCOK) {
      release_latch(c);
      cleanup(c, last_ex);
...
RC index_btree::insert_into_leaf(glob_param params, bt_node *leaf,
                                 idx_key_t key, itemid_t *item) {
...
RC index_btree::split_lf_insert(glob_param params, bt_node *leaf, idx_key_t key,
                                itemid_t *item) {
  RC rc;
...
RC index_btree::insert_into_parent(glob_param params, bt_node *left,
...
RC index_btree::insert_into_new_root(glob_param params, bt_node *left,
                                     idx_key_t key, bt_node *right) {
  RC rc;
  uint64_t part_id = params.part_id;
  bt_node *new_root;
...
RC index_btree::split_nl_insert(glob_param params, bt_node *old_node,
                                UInt32 left_index, idx_key_t key,
                                bt_node *right) {
  RC rc;
        ...
        do {
                ...
                while (c != NULL) {
                        for (int i = 0; i < c->num_keys; i++) {
                                row_t * r = (row_t
*)((itemid_t*)c->pointers[i])->location; if (c->is_leaf)
                                        printf("%lld(%lld,%d),",
                                                c->keys[i],
                                                r->get_uint_value(0),
