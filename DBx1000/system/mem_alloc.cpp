...
#else
  pid_arena = new std::pair<uint64_t , int>[_bucket_cnt];
#endif
...
void *Arena::alloc() {
  ...
  if (_head == NULL) {
    ...
    int size = (_block_size + sizeof(FreeBlock) + (MEM_ALLIGN - 1)) &
               ~(MEM_ALLIGN - 1);
...
#else
    uint64_t pid;
    pthread_threadid_np(nullptr, &pid);
    uint64_t entry = pid % _bucket_cnt;
#endif
